...
void GPIO_ModePushPull(GPIO_TypeDef *port, int32_t linia){
	...
	// Fixem el registre PUPDR sense PP ni PD, a '00'
	port->PUPDR = (port->PUPDR)&(~BIT(2*linia))&(~BIT(2*linia+1));
}
static void lcdGPIOInit(void){
	uint8_t linies[] = {LCD_BL_PAD, LCD_E_PAD, LCD_RS_PAD, LCD_DB4_PAD,
...
static void lcdNibble(int32_t nibbleCmd, int32_t RS) {
	...
	// Fer pols de senyal d'habilitació (E)de 10us
	DELAY_US(10);
	// Posem E a '1'
...
void LCD_ClearDisplay(void){
	lcdNibble(0,0); //DB7... DB4 a '0000' i RS = 0
	lcdNibble(1,0); //DB3... DB0 a '0001' i RS = 0
	DELAY_US(1000);
}
...
void LCD_Config(int32_t Disp,int32_t Cursor,int32_t Blink){
	...
	if(Blink!=0) Blink = 1;
	//Creem els 4 bits de nivell baix que son del tipus '1DCB'
...
// This code is requested in P2.7
	lcdNibble(car>>4,1); // 1a transferència (bits alts)
	lcdNibble(car,1); // 2a transferència (bits baixos)
...
void LCD_SendString(char *string) {
	// This code is requested in P2.7
	uint32_t i = 0;
	// Enviem l'string caràcter a caràcter
	// Mentre no trobem el seu final
	while(string[i]!=0){
...
void LCD_CustomChar(int32_t pos, uint32_t *data){
	int32_t caracter = ((1<<6)|(pos<<3)); // Indiquem quin caràcter del 0 al 7 volem
	// Apuntem a la memòria CGRAM
	LCD_SendChar(caracter);
	int32_t i;
	for(i = 0; i < 8; i++){ //recorrem el vector data
		LCD_SendChar(data[i]);
	}
}
